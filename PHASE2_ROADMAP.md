...
**Remaining for a full build-out:** document versioning/replace/delete, vendor participation in documents, document-level comment threads, group chat, real per-project permission tiers, project notifications (comments/tasks/documents currently notify no one), and a decision on whether/how vendors ever participate more broadly. **Revised estimate for the remainder:** roughly 3–5 weeks (the entity/membership/task/comments/activity/documents groundwork is now done).
