...
| Task assignment to suppliers + completion tracking | Medium–High | ⚠️ **Partially done, rescoped** — `project_tasks` ships with assignment + status tracking, but only to **internal profiles** (admin/manager/staff). Vendor task assignment (`project_vendor_tasks`) was explicitly scoped for Vendor Project Access but deferred to **v1b** — not part of §6-H. No dependency-graph engine, as originally scoped. |
...
**Vendor access, since §6-H:** `/vendor-projects` (list) and `/vendor-projects/:id` (detail) — vendor-only routes, deliberately separate pages from the internal Project Detail rather than a conditional branch inside it. A vendor sees a safe six-column project summary (via RPC, no direct `projects` table access), documents/comments explicitly shared with them, and their own linked appointments. See §6-H for the full record, including the two accepted risks (staff has no vendor-roster visibility yet; the doc-sharing INSERT policy isn't DB-restricted to admin/manager, only UI-restricted).

**Remaining for a full build-out:** document versioning/replace/delete, document-level (not just per-vendor) comment threads, group chat, real per-project permission tiers, email/push notifications for project events (including vendor-triggered ones, which currently notify no one), vendor task assignment (`project_vendor_tasks`, planned as v1b), and staff visibility into the vendor roster. **Revised estimate for the remainder:** roughly 2–4 weeks (the entity/membership/task/comments/activity/documents/notifications/vendor-access-v1a groundwork is now done).
