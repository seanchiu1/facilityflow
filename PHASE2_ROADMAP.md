...
| Comment thread on documents | Medium | ⚠️ **Adjacent feature built** — project-level comments + an activity timeline shipped (`project_comments`/`project_activity`, see §6-E), but these are comments on the *project*, not threaded comments on a *document* (no project documents exist yet). The original document-comment spec remains unbuilt. |
...
**What "Lite" means concretely:** `/projects` (list) and `/projects/:id` (detail) for admin/manager/staff — vendor has no route, nav item, or RLS grant. Admin/manager get full CRUD on projects/members/tasks; staff get read access scoped to their own memberships (RLS-enforced) and change their own task's status via a column-scoped RPC. Project Detail now also carries **comments** (members + managers, immutable v1) and an **activity timeline** (project created, status changed, member added, task created/status changed, appointment linked — app-written after each successful action, plus RPC-written for staff task changes). Appointment Detail gained an admin/manager-only "Project" link dropdown, with a nullable, additive `appointment_requests.project_id` — existing appointments are unaffected. See §6-D/§6-E for the full acceptance-criteria records and accepted risks.

**Remaining for a full build-out:** document library, document-level comment threads, group chat, real per-project permission tiers, project notifications (comments/tasks currently notify no one), and a decision on whether/how vendors ever participate. **Revised estimate for the remainder:** roughly 3–6 weeks (the entity/membership/task/comments/activity groundwork is now done).
