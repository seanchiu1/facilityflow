...
| Document library incl. vendor-maintained Gantt file uploads | Medium | ⚠️ **Partially done, rescoped** — Project Documents v1 (§6-F) shipped admin/manager/staff-member upload+view; **Vendor Project Access v1a (§6-H) added vendor-visible document sharing on top of it** — a document can now be tagged `visibility='vendor'` and shared with exactly one vendor, who can view it (and upload their own) under a dedicated storage prefix. No versioning, replace/delete, or Gantt-specific handling. |
| Comment thread on documents | Medium | ⚠️ **Adjacent feature built** — project-level comments (§6-E) plus, since §6-H, a **per-vendor shared comment thread** (not a per-document thread — the original document-comment spec remains unbuilt). |
| Group chat across stakeholders | High | ⏸ Not started for internal-to-internal chat. §6-H added a narrower thing — a one-vendor-at-a-time shared thread, refresh-based, not a group chat and not vendor-to-vendor. |
| Task assignment to suppliers + completion tracking | Medium–High | ✅ **Done for vendors, since v1b** — `project_vendor_tasks` (see §6-I) lets admin/manager assign a task to a specific vendor; the vendor sees only their own tasks and changes only status via RPC. Internal `project_tasks` remains a separate table for internal profiles, unaffected. No dependency-graph engine, as originally scoped. |
| Multiple vendors participating in a project, safely isolated from each other | High | ✅ **Done (v1a)** — see §6-H and `supabase_vendor_project_access_v1a_migration.sql`. This item wasn't in the original §6-B list but emerged as a hard prerequisite once vendor access was actually built: vendors are added via a separate `project_vendor_members` table (never `project_members`), and Vendor A cannot detect Vendor B on the same project through any table, RPC, or embed. |

**What "Lite" means concretely:** `/projects` (list) and `/projects/:id` (detail) for admin/manager/staff — vendor has no route, nav item, or RLS grant on the internal tables. Admin/manager get full CRUD on projects/members/tasks; staff get read access scoped to their own memberships (RLS-enforced) and change their own task's status via a column-scoped RPC. Project Detail now also carries **comments** (members + managers, immutable v1), an **activity timeline** (project created, status changed, member added, task created/status changed, appointment linked, document uploaded — app-written after each successful action, plus RPC-written for staff task changes), and **documents** (upload/view only, immutable, member-scoped via `project_documents` RLS, reusing the existing private storage bucket with no storage-policy changes). Appointment Detail gained an admin/manager-only "Project" link dropdown, with a nullable, additive `appointment_requests.project_id` — existing appointments are unaffected. See §6-D/§6-E/§6-F for the full acceptance-criteria records and accepted risks.

**Notifications, since §6-G:** project events (task assigned, task status changed, comment added, document uploaded, member added, appointment linked) now raise an in-app notification via the existing Topbar bell — see `supabase_project_notifications_migration.sql` and `PHASE2_REQUIREMENTS.md` §6-G. In-app only, no email/push. Vendor-triggered events (a vendor's own upload or reply) do **not** raise notifications or activity entries — both gate on `is_internal_role()`, which vendors fail by construction.

**Vendor access, since §6-H:** `/vendor-projects` (list) and `/vendor-projects/:id` (detail) — vendor-only routes, deliberately separate pages from the internal Project Detail rather than a conditional branch inside it. A vendor sees a safe six-column project summary (via RPC, no direct `projects` table access), documents/comments explicitly shared with them, and their own linked appointments. See §6-H for the full record, including the two accepted risks (staff has no vendor-roster visibility yet; the doc-sharing INSERT policy isn't DB-restricted to admin/manager, only UI-restricted — both closed for tasks specifically in §6-I below).

**Vendor tasks, since §6-I:** `project_vendor_tasks` — a table separate from `project_tasks` — lets admin/manager assign tasks to vendors on the project's roster; the vendor's "My Tasks" section on `/vendor-projects/:id` shows only their own tasks and can change only status, via `update_my_vendor_project_task_status()`. A validation trigger prevents assigning a task to a vendor who isn't actually a `project_vendor_members` row on that project.

**Vendor notifications, since §6-J:** the in-app bell now covers vendor collaboration both directions — a vendor is notified when assigned a task, replied to, or shared a document with; the internal team is notified when a vendor comments, uploads, or changes a task's status. Two new SECURITY DEFINER RPCs (`notify_vendor_project_event()`, `notify_internal_vendor_project_event()`), kept separate from the §6-G internal RPCs rather than widening them. Still in-app only, still no vendor activity feed — see §6-J for the full record.

**Remaining for a full build-out:** document versioning/replace/delete, document-level (not just per-vendor) comment threads, group chat, real per-project permission tiers, email/push notifications for project events (in-app-only for both internal and vendor collaboration as of §6-J), a vendor activity feed, a staff-facing vendor roster UI, and vendor task reassignment/deletion UI. **Revised estimate for the remainder:** roughly 2–3 weeks (the entity/membership/task/comments/activity/documents/notifications/vendor-access/vendor-tasks/vendor-notifications groundwork is now done).
