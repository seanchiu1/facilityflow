...
**Vendor tasks, since §6-I:** `project_vendor_tasks` — a table separate from `project_tasks` — lets admin/manager assign tasks to vendors on the project's roster; the vendor's "My Tasks" section on `/vendor-projects/:id` shows only their own tasks and can change only status, via `update_my_vendor_project_task_status()`. A validation trigger prevents assigning a task to a vendor who isn't actually a `project_vendor_members` row on that project. No vendor task notifications yet — see §6-I's accepted risks.

**Remaining for a full build-out:** document versioning/replace/delete, document-level (not just per-vendor) comment threads, group chat, real per-project permission tiers, email/push notifications for project events (including vendor-triggered and vendor-recipient ones, which currently notify no one), a staff-facing vendor roster UI, and vendor task reassignment/deletion UI. **Revised estimate for the remainder:** roughly 2–3 weeks (the entity/membership/task/comments/activity/documents/notifications/vendor-access/vendor-tasks groundwork is now done).
