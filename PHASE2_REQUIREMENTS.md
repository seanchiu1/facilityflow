...
- **Wave 2 (later production):** email version — ✅ **implemented and live in production** (Bucket 3, L-1: `send-notification-emails` Edge Function + `notification_logs` table). See the L-1 note below.

**L-1 email infrastructure — ✅ IMPLEMENTED and live in production:** see `supabase_l1_notification_logs_migration.sql` and `supabase/functions/send-notification-emails/index.ts`. The Edge Function reuses this same 1-hour-window logic server-side, is guarded by a required `x-notification-secret` header (tested: a request without it returns `401` before any query runs), and returns `503` if `RESEND_API_KEY`/`RESEND_FROM_EMAIL` aren't configured — confirmed no `notification_logs` row is written and no email is attempted in that case. **A Resend account (API key + verified sender/domain) is configured, and a daily GitHub Actions workflow (`.github/workflows/facilityflow-email-cron.yml`) schedules sends — not `pg_cron`.** `notification_logs` has confirmed `sent` rows.
...
- **Wave 2 (later production):** scheduled email version — ✅ **implemented and live in production** (Bucket 3, L-1), scheduled by a daily GitHub Actions workflow, not `pg_cron` — see the L-1 note under §4-B for the full record, which applies identically here (both notification types share the same `send-notification-emails` function and `notification_logs` table).
