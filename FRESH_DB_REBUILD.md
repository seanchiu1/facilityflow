...
| 24 | `supabase_demo_seed.sql` | **Seed (optional)** | Steps 0, 1, 9, 13 + manager/vendor accounts | Appointment workflow demo data |
| 25 | `supabase_demo_seed_projects.sql` | **Seed (optional)** | Step 24 + Steps 16, 17, 22, 23 + all 5 demo accounts | Project/vendor collaboration demo data |
...
Run via `supabase db advisors --linked --type all` against `kwelwlnsxmgazhfzpeqo` as part of this pass — read-only, nothing changed. **None of these are fixed in this pass** (out of scope: "do not change RLS unless a rebuild bug requires it," and none of them block a rebuild — a fresh project built from §2's file list will reproduce every one of these exact findings, which is expected, not a regression). Listed here because a fresh rebuild should be judged against a known baseline, not a surprise.

1. **24 of 26 `SECURITY DEFINER` functions are executable by the unauthenticated `anon` role at the Postgres grant level**, despite every migration file's `revoke all on function ... from public` statement. Root cause: Supabase projects configure `ALTER DEFAULT PRIVILEGES` to auto-grant `EXECUTE` to `anon`/`authenticated` on newly created functions in the `public` schema — a `REVOKE ... FROM PUBLIC` does not undo that separate, role-specific grant; it needs an explicit `REVOKE ... FROM anon`. **Verified not currently exploitable**: every flagged function either (a) is a trigger function unreachable via PostgREST RPC (`enforce_*`, `sync_project_owner_membership` — 5 of the 24), or (b) keys its logic off `auth.uid()`/`current_profile_role()`, both of which are `NULL` for an anon caller, so every data-returning or data-writing function among the remaining 19 fails closed (0 rows returned, 0 rows affected, or an explicit role-check exception) when called anonymously. This is a real defense-in-depth gap worth closing, not a confirmed live vulnerability. Fix pattern for a future pass: `revoke execute on function public.<name> from anon;` added alongside the existing `revoke ... from public` line, for all 24.
2. **1 `SECURITY DEFINER` view** (`slot_booking_counts`, from `supabase_rls_prep_migration.sql`) — flagged as `ERROR` level by the linter. It aggregates `appointment_requests` (`responsible_staff`, `requested_date`, `start_time`, booked count) with no `WHERE` clause restricting to the querying user, and no RLS ever applied to it in the first place, so `SECURITY DEFINER` vs. `SECURITY INVOKER` is moot for the *data* shown here (a booking-capacity count, already intentionally cross-vendor so the booking form can show "how many slots are taken," not per-user data) — but the linter is right that this is worth an explicit look before treating it as fine forever.
3. **2 functions lack `set search_path`** (`fn_set_appointment_code`, `set_updated_at`) — both predate the "always `set search_path = public`" convention established partway through this project's migrations. Low risk (both are narrow trigger functions with no dynamic SQL), but inconsistent with every function written since.
4. **20 RLS policies re-evaluate `auth.uid()`/`auth.role()` per row** instead of once per query (`auth_rls_initplan`) — a widely-documented Supabase performance optimization (wrap in `(select auth.uid())` so Postgres caches it as an initplan). Performance-only, not a correctness or security issue.
...
**Recommendation:** treat item 1 (`anon` execute grants) as the one worth a dedicated follow-up pass — it's cheap to fix (24 one-line `REVOKE` statements) and closes a real defense-in-depth gap even though nothing is currently exploitable through it. Items 4–5 are pure performance tuning, worth doing before a real production load, not before a demo. Item 2 deserves a five-minute look, not urgent. Item 6 is a Dashboard checkbox whenever this goes anywhere near real user passwords.
