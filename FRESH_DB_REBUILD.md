...
| 26 | `supabase_staff_profile_linking_migration.sql` | Migration | Steps 10 (`profiles`), 7 (`staff_schedules`), 12 (`duty_rosters`) | Adds nullable `staff_profile_id` on `staff_schedules` and `duty_staff_profile_id` on `duty_rosters` — found while debugging why a Duty Roster entry didn't open a vendor booking slot: Schedule Management's staff picker was hardcoded to 5 fictional demo names, unrelated to real `profiles` rows. Purely additive (no RLS change) — see `BOOKING_AVAILABILITY_DEBUG.md`. |
| 27 | `supabase_booking_availability_rule_migration.sql` | Migration | Step 25 (redefines the function it created) | Product rule change: staff aren't equipment specialists and have no per-vendor capacity limit. Redefines `get_available_schedule_slots()` — same name and signature — to drop its `equipment_type` filter; availability now depends on date only. No RLS change; `capacity`/`equipment_type` columns are left in place but unused for availability going forward. See `BOOKING_AVAILABILITY_DEBUG.md` §9. |
| — | *Manual:* remaining demo accounts if not already created (§3) | Manual | — | Needed only if you're about to run the seeds below |
| 28 | `supabase_demo_seed.sql` | **Seed (optional)** | Steps 0, 1, 9, 13 + manager/vendor accounts | Appointment workflow demo data |
| 29 | `supabase_demo_seed_projects.sql` | **Seed (optional)** | Step 28 + Steps 16, 17, 22, 23 + all 5 demo accounts | Project/vendor collaboration demo data |
