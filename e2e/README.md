...
| A | Manager creates one `staff_schedules` slot via Schedule Management (live active-profile staff picker, not a hardcoded list), tagged with one specific equipment type | `booking-availability.spec.js` |
| B | `vendor@` sees that exact slot on the Booking Form while browsing with a **different** equipment type selected, confirms switching equipment doesn't clear the selected slot, then completes a real booking submission | `booking-availability.spec.js` |
| C | `vendor2@` — a second, independent vendor — also sees and can select the exact same slot, browsing with a **third** equipment type selected; nothing marks it "full" | `booking-availability.spec.js` |
| D | `vendor@` cannot read `staff_schedules` directly via a raw REST call (same request shape a vendor's browser could make from devtools) | `booking-availability.spec.js` |

Tests A–C are the end-to-end version of the booking availability rule in `BOOKING_AVAILABILITY_DEBUG.md` §9: staff aren't equipment specialists and have no per-vendor capacity limit, so a slot tagged one equipment type must still be visible, selectable, and bookable by any vendor browsing with any other equipment type — and by more than one vendor at once. Test D is the browser-level equivalent of `VENDOR_ISOLATION_AUDIT.md`'s SQL check for the same table, run through the actual deployed API instead of a simulated database session.
...
`booking-availability.spec.js` is the one exception: it creates a real `staff_schedules` row (as the manager account, equipment type `HVAC`, next calendar week, a `notes` value unique to that test run — e.g. `E2E-SLOT-1732982400000`) and, if test B reaches submission, one real `appointment_requests` row (vendor name `E2E Test Vendor <run id>`). Both are cleaned up in an `afterAll` hook regardless of whether earlier assertions passed: the appointment is set to **Cancelled** via the same Update Status action a manager would use (not a database delete), and the schedule slot is deleted via Schedule Management's own delete button. It's still safe to run against production — everything created is uniquely tagged with the test run's timestamp specifically so it can never be confused with real data — but it is a write, unlike every other file here. If you want a strictly read-only run (e.g., during an active pilot demo you don't want any risk of touching), name the other files explicitly instead of the full suite: `npx playwright test e2e/manager.spec.js e2e/vendor.spec.js e2e/routing.spec.js`.
...
- **`booking-availability.spec.js`'s appointment cleanup depends on the Requests page search matching by vendor name** — if that ever stops working (e.g. the search field changes), the `afterAll` hook logs an error to the console rather than throwing, and the test run still passes/fails on its own assertions; check the console output after a run and cancel `E2E Test Vendor <run id>` manually via **Requests** if the automated cleanup didn't find it.
