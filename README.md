...
| **Vendor Project Access (v1a/v1b)** | `/vendor-projects` (vendor-only, separate pages from the internal Project Detail) — lets multiple vendors participate in a project without ever joining the internal collaboration surface: vendors are added via a separate `project_vendor_members` table (never `project_members`), see a six-column project summary via SECURITY DEFINER RPC (no description/owner/created_by), documents/comments explicitly shared with them (`visibility='vendor'`/`'shared'`, isolated per vendor), their own linked appointments, and **(v1b) their own tasks** — admin/manager assign a task from a separate `project_vendor_tasks` table, the vendor changes only its status via `update_my_vendor_project_task_status()`. Internal `ProjectDetail.jsx` gained Vendors and Vendor Tasks cards (admin/manager) for add/remove, per-vendor shared threads, document-sharing controls, and task assignment. **Vendor A cannot detect Vendor B on the same project through any table, RPC, or embed.** No vendor activity/notifications, no vendor-to-vendor visibility (permanent, not a gap). See PHASE2_REQUIREMENTS.md §6-H/§6-I |
...
21. Optionally insert sample schedule slots
