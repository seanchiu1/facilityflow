...
| **Project Collaboration Lite** | `/projects` (admin, manager, **and** staff — vendor excluded in v1) — lightweight tracking for longer-running facilities work: projects with a site/owner/status/dates, a members list, a task list (assign to internal profiles, staff can update the status of their own tasks), and a read-only list of linked appointments. Admin/manager link an appointment to a project from Appointment Detail. **This is a first "Lite" slice, not a full project management suite** — no document library, comments, or group chat yet; see PHASE2_REQUIREMENTS.md §6-D for exactly what's built versus still aspirational |
...
15. Run `supabase_projects_lite_migration.sql` (`projects`/`project_members`/`project_tasks` tables, `project_id` on `appointment_requests`, `is_project_member()` helper, RLS) — required for `/projects` and the Project dropdown in Appointment Detail
16. Optionally insert sample schedule slots
