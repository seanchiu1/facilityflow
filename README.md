...
| **Project Collaboration Lite** | `/projects` (admin, manager, **and** staff — vendor excluded in v1) — lightweight tracking for longer-running facilities work: projects with a site/owner/status/dates, a members list, a task list (assign to internal profiles, staff update their own task status via a column-scoped RPC), a read-only list of linked appointments, **project comments** (members + managers, immutable v1), and an **activity timeline** (project created, status changed, member added, task created/status changed, appointment linked). Admin/manager link an appointment to a project from Appointment Detail. **This is a "Lite" slice, not a full project management suite** — no document library, group chat, realtime, or notifications; the activity feed is app-written and best-effort, not a tamper-proof audit log. See PHASE2_REQUIREMENTS.md §6-D/§6-E |
...
17. Optionally insert sample schedule slots
